export const SELECT_JOB_DUE_DATE = "SELECT_JOB_DUE_DATE";
export const SELECT_JOB_START_DATE = "SELECT_JOB_START_DATE";
export const SELECT_JOB_End_DATE = "SELECT_JOB_End_DATE";

/* starting types for subscription page */

export const INCREASE_NUMBER_OF_USERS_BY_ONE =
  "INCREASE_NUMBER_OF_USERS_BY_ONE";
export const DECREASE_NUMBER_OF_USERS_BY_ONE =
  "DECREASE_NUMBER_OF_USERS_BY_ONE";
export const SET_NUMBER_OF_USERS = "SET_NUMBER_OF_USERS";

/* Starting action for addStaff Member */

export const ADD_STAFF_MEMBER_START_DATE = "ADD_STAFF_MEMBER_START_DATE";

/* End action for addStaff Member */

// Start Send invoice Action Types //

export const REMOVE_INVOICE_FILE = "REMOVE_INVOICE_FILE";
export const ADD_INVOICE_FILE = "ADD_INVOICE_FILE";

// Start Quote Upload Files  Action Types //

export const ADD_QUOTE_FILE = "ADD_QUOTE_FILE";
export const REMOVE_QUOTE_FILE = "REMOVE_QUOTE_FILE";
export const CHANGE_QUOTE_FILE_DESCRIPTION = "CHANGE_QUOTE_FILE_DESCRIPTION";

// actions for Scheduler View
export const CHANGE_SCHEDULER_VIEW = "CHANGE_SCHEDULER_VIEW";

// actions for authentication

export const SET_USER = "SET_USER";
